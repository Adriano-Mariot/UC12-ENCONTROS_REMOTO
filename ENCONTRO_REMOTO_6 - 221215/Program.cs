...
do
{
    ...
    switch (op1)
    {
        case "0":
            ...
            break;
        case "1":
                Utils.loading("Carregando Dados da Pessoa Fisica ",3,300);            
                ...
                Nome = {pf1.Nome}
                Cpf = {pf1.Cpf}
                Rendimentos Anual= {pf1.Rendimento}
                Rendimentos Liquido = {metodosPf.PagarImposto(pf1.Rendimento)}
                Idade = {pf1.DataNaceimento.ToShortDateString()}
                Maior de Idade? = {metodosPf.ValidarDataNascimento (pf1.DataNaceimento)}                
                ");             

                Console.WriteLine(@$"
                ------Endereço da Pessoa Física-------
                Logradouro = {pf1.Endereco.Logradoura},{pf1.Endereco.Numero}
                Endereço comercial? : {pf1.Endereco.Comercial}
                ");
               
                Utils.ParadaNoConsole(" ");
            break;
        case "2":
                Utils.loading("Carregando Dados da Pessoa Jurídica ",3,300);         
